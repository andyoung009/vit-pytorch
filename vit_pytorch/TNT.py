...
# 这是一个Python函数，它接受三个参数：`v`，`divisor`和`min_value`。
# 它返回一个新值，该新值是`divisor`参数的倍数，但也大于或等于`min_value`参数。# 这确保了新值至少为`min_value`并且是`divisor`的倍数。
def make_divisible(v, divisor=8, min_value=None):
    # 该函数首先将`min_value`的默认值设置为与`divisor`参数相同。然后
    min_value = min_value or divisor
    # 通过将`divisor/2`加到`v`上，向下舍入到最接近`divisor`的倍数，然后取该值和`min_value`的最大值来计算`v`的新值。
    new_v = max(min_value, int(v + divisor / 2) // divisor * divisor)
    # Make sure that round down does not go down by more than 10%.
    # 接下来，函数检查新值是否小于原始值`v`的90％。如果是，则再添加一个`divisor`的倍数到新值，以确保新值不太小于原始值。最后，该函数返回新值。
    if new_v < 0.9 * v:
        new_v += divisor
    return new_v
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, outer_dim=768, inner_dim=24, inner_stride=4):
        ...
        # nn.unfold()是PyTorch中的一个函数，用于将一个二维的输入张量展开成一个三维的输出张量。
        # 具体来说，它将输入张量的每个滑动窗口展开成一个列向量，并将这些列向量按照顺序排列成一个矩阵.
